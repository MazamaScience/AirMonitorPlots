...
spokanePlot + geom_pm25Points(aes(color = monitorID)) + stat_nowcast(aes(color = monitorID))
spokanePlot + stat_dailyAQILevel(timezone = "America/Los_Angeles") + facet_grid(rows = vars(monitorID))
spokanePlot + stat_AQILevel(color = NA) + stat_dailyAQILevel(alpha = .5, timezone = "America/Los_Angeles") + facet_grid(rows = vars(monitorID)) 
spokanePlot + custom_aqiStackedBar(width = 1, alpha = .3) + stat_nowcast(aes(linetype = monitorID))
spokanePlot + custom_aqiLines() + stat_nowcast(aes(linetype = monitorID)) + custom_pm25TimeseriesScales(spokane, startdate = 20150810, enddate = 20150820) 
